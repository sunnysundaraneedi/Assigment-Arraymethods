...
let result1 = superMarket.filter((item) => {
  return item.type === "dairy";
});

let averageRatings = result1.reduce((previousValue, currentValue) => {
  return previousValue + currentValue.rating / result1.length;
}, 0);
// -----------------------------------------------

let result2 = superMarket.filter((item) => {
  return item.type === "dairy";
});

let averagePrice = result2.reduce((previousValue, currentValue) => {
  return previousValue + currentValue.price / result2.length;
}, 0);
averagePrice = Math.ceil(averagePrice);
// -----------------------------------------------

let result3 = superMarket.filter((item) => {
  return item.type === "fruit";
});
result3.map((item) => {
  return (item.isAvailable = true);
});
result3 = result3.map((result) => result.title);
// -----------------------------------------------

let result4 = superMarket.filter((item) => {
  return item.type === "dairy" || item.type === "vegetable";
});
result4.map((item) => {
  if (item.type === "dairy") {
    return (item.isAvailable = true);
...
document.write(`The elements of type: 'dairy' and averageRatings: ${averageRatings}<br><hr>
                The elements of type: 'vegetable' and calculate averagePrice: ${averagePrice}<br><hr>
                The elements of type: 'fruit' with new property 'isAvailable: true': ${result3}<br><hr>
                The elements of type: 'dairy' and 'vegetable' and added new property 'isAvailable: true'
                to the filtered elements under type: 'dairy' and added new property 'isAvailable: false' to the 
                filtered elements under type: 'vegetable', result can be seen in the console
`);
